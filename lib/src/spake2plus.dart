...
class Spake2plus {
  ...
  late final ffi.Pointer<BN_CTX> _ctx;
  late final ffi.Pointer<EC_GROUP> _group;
  late final ffi.Pointer<BIGNUM> _order;
  ...
  Spake2plus(String libraryPath, List<int> curvePointM, List<int> curvePointN)
    ...
    _ctx = _openssl.BN_CTX_new();
    _group = _openssl.EC_GROUP_new_by_curve_name(NID_X9_62_prime256v1);
    _order = _openssl.BN_new();
    final res = _openssl.EC_GROUP_get_order(_group, _order, _ctx);
    assert(res == 1);
  }

  (Uint8List, Uint8List) computeWitnesses(ScryptParameters scryptParams) {
    return _computeWitnesses(scryptParams, _group, _ctx);
  }
  ...
  (Uint8List, Uint8List) _computeWitnesses(
    ...
    _openssl.calculateW(w0, z0, _order, ctx);
    _openssl.calculateW(w1, z1, _order, ctx);

    assert(_openssl.BN_cmp(w0, w1) == 1);

    final w0Bytes = _openssl.bn2bytes(_group, w0, 32, _ctx);
    final w1Bytes = _openssl.bn2bytes(_group, w1, 32, _ctx);
  ...
  Uint8List randomValidScalar() {
    ...
    _openssl.BN_rand_range(p, _order);
    final ret = _openssl.bn2bytes(_group, p, 32, _ctx);
    _openssl.BN_free(p);
    return ret;
  }
  ...
  Uint8List computeShareV(Uint8List w0Bytes, Uint8List yBytes) {
    ...
    final bytesY = _openssl.point2bytes(_group, Y, _ctx);
  ...
  Uint8List computeRecordL(Uint8List w1Bytes) {
    ...
    final bytesL = _openssl.point2bytes(_group, L, _ctx);
  ...
  (Uint8List Z, Uint8List V)? verifierFinish(
      Uint8List w0Bytes, Uint8List bytesL, Uint8List bytesX, Uint8List yBytes) {
    final group = _group;
    final ctx = _ctx;
  ...
  Uint8List _digest(String sn, Uint8List input) {
    ...
    final hash = calloc<ffi.UnsignedChar>(256);
  ...
  Uint8List convertToUncompressedECPoint(Uint8List pointData) {
    final cp = _openssl.EC_POINT_new(_group);
    final buf = pointData.toPointer();
    _openssl.EC_POINT_oct2point(_group, cp, buf, pointData.length, _ctx);

    final len = 65;
    final ptr = calloc<ffi.UnsignedChar>(len);
    _openssl.EC_POINT_point2oct(_group, cp,
        point_conversion_form_t.POINT_CONVERSION_UNCOMPRESSED, ptr, len, _ctx);
